...
# willemsma.it/synced-secret-operator-bundle:$VERSION and willemsma.it/synced-secret-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= willemsma.it/synced-secret-operator
...
.PHONY: docker-buildx
docker-buildx: ## Build docker image with the manager multi arch
	docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v7 -t ${IMG} --push .
